...
type Props = {
  ...
  initialTitle?: string;
  initialField: string;
  onSubmit: (patch: { title?: string; field: string }) => void;
  onCancel: () => void;
};

export function ExistsForm({ sectionMode, initialTitle, initialField, onSubmit, onCancel }: Props) {
  ...
  return (
    <div className={`rounded-md border-2 border-dashed ${meta.softBorder} ${meta.softBg} p-3`}>
      ...
      <input
        ...
        placeholder="e.g. user-id is set"
        spellCheck={false}
      />
      ...
      <div className="mt-2 flex items-center justify-end gap-2">
        ...
        <button
          onClick={() => canSave && onSubmit({ title: title.trim() || undefined, field: field.trim() })}
          disabled={!canSave}
